...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mods="http://www.loc.gov/mods/v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    ...
    <xsl:template match="RECORD" mode="record">
        ...
        <mods:mods>
            ...
            <mods:note type="doc type">Article</mods:note> <!--on the same line to eliminate spaces -->
            
            <!-- Authors -->
            <xsl:apply-templates select="ANALYTIC_LEVEL_author_s_">
    ...
    <!-- Document Type A Specific Layout -->
    <xsl:template match="RECORD" mode="A">
        <mods:originInfo>
        ...
        <mods:relatedItem type="host">
            <mods:originInfo>
                ...
                <xsl:apply-templates select="Date_of_publication" >
                    <xsl:with-param name="destination">relatedItem</xsl:with-param>
                </xsl:apply-templates>
            </mods:originInfo>                                      
            <xsl:apply-templates select="Serial_Journal_title" />
            <xsl:apply-templates select="Collation" mode="part" />
            <xsl:apply-templates select="Other_pagination[not(../Collation)]" mode="part" />
            <xsl:apply-templates select="ISBN" />
        </mods:relatedItem>
    
    </xsl:template>
    <!-- End Document Type A Specific Layout -->
    ...
    <xsl:template match="MONOGRAPHIC_LEVEL_author_s_ | Corporate_author_s_ | Monographic_source_author_s_ | Corporate_source_author_s_">
        <xsl:param name="sType" />
        <xsl:param name="sRole" />
        <xsl:param name="sDocType" />
        <mods:name>
